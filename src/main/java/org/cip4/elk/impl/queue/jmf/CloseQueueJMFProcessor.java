...
public class CloseQueueJMFProcessor extends AbstractJMFProcessor {
    ...
    private static final String MESSAGE_TYPE = "CloseQueue";
    
    public CloseQueueJMFProcessor(Config config, Queue queue) {        
        super();
    ...
    /* (non-Javadoc)
     * @see org.cip4.elk.impl.jmf.AbstractJMFProcessor#processMessage(com.heidelberg.JDFLib.jmf.JDFMessage, com.heidelberg.JDFLib.jmf.JDFResponse)
     */
    public int processMessage(JDFMessage input, JDFResponse output) {
        return processCloseQueue((JDFCommand)input, output);
    }

    public int processCloseQueue(JDFCommand command, JDFResponse response) {
        int returnCode = 0;
